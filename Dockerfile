...
RUN \
    ...
    tar xf rav1e.tar.gz && \
    cd rav1e-* && /root/.cargo/bin/cargo cinstall --release && \
    RUSTFLAGS="-C target-feature=+crt-static" /root/.cargo/bin/cargo cinstall --release
...
ARG LCMS_VERSION=2.15
ARG LCMS_URL="https://github.com/mm2/Little-CMS/releases/download/lcms$LCMS_VERSION/lcms2-$LCMS_VERSION.tar.gz"
ARG LCMS_SHA256=b20cbcbd0f503433be2a4e81462106fa61050a35074dc24a4e356792d971ab39
RUN \
    wget $WGET_OPTS -O lcms2.tar.gz "$LCMS_URL" && \
    echo "$LCMS_SHA256  lcms2.tar.gz" | sha256sum --status -c - && \
    tar xf lcms2.tar.gz && \
...
RUN \
    ...
    tar xf ffmpeg.tar.bz2 && cd ffmpeg-* &&  \
    ...
    --enable-libxvid \
    --enable-libzimg \
    --enable-openssl || (cat ffbuild/config.log ; false) \
    && make -j$(nproc) install
